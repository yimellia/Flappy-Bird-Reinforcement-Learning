...
class Bot:
    """
    The Bot class is the agent that applies the Q-learning logic to Flappy bird game.
    After every iteration (i.e. one game that ends with the bird dying), it updates Q values.
    """
    ...
    def __init__(self):
        ...
        self.lr = 0.7               # learning rate (i.e. alpha)
        self.load_qvalues()
    ...
    def showStep(self, num, state, action, new_state):
        print(f"{0:5d}: ({1:15s}:{2:6d}, {3:1d}): {4:6.0f} => ({5:15s}): {6:6.0f}, {7:6.0f}, {8:6d}",
            num+1, state, self.qvalues[state][2], action, self.qvalues[state][action],
    ...
    def act(self, x, y, vel, pipe):
        """
        Choose the best action with respect to the current state - Choose 0 (don't flap) to tie-break
        """
    ...
    def initStateIfNull(self, state):
        if self.qvalues.get(state) == None:
             ...
             if num > 30000:
                print(f"======== New state: {state:14s}, Total: {num} ========")
